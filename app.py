...
inferencer = MMPoseInferencer(pose2d='human', pose2d_weights='rtmpose-m_simcc-body7_pt-body7_420e-256x192-e48f03d0_20230504.pth', det_model='whole_image')
import cv2
app = Flask(__name__)
...
def upload_file():
    ...
    print("FILENAME OF THE UPLOADS FOLDER: ", filename)
    file.save(filename)
    if not detect_faces(filename):
        return jsonify({"Error": "Input is not a persons image"})

    result_generator = inferencer(filename, out_dir = "output", return_vis=True, radius=12, thickness=5, draw_bbox=False, draw_heatmap=False)
    ...
    if y_pred==0:
        ...
        #print("FILENAME OF POSTURE FILE: ", filename)
        shutil.copy2(filename, "./improper")

    elif y_pred==1:
        print("PROPER POSTURE")
        posture_type = "proper"
        #filename = os.path.join("proper", file.filename)
        shutil.copy2(filename, "./proper")
